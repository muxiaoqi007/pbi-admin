...
| 数据集 | 全租户数据集浏览，查看数据源、**反查使用该数据集的前端报表**（含跨工作区引用）、刷新记录 |
| 刷新 | 触发刷新：全部刷新（经典）或选表刷新（增强刷新，可配 full/automatic、提交模式、并行度、重试次数） |
...
首次使用请进入「设置」页：选择云环境 → 填写租户 ID、客户端 ID、客户端密钥 → 点「保存并测试连接」。测试通过后其余页面即可正常使用。

生产部署：`npm run build && npm start`。工具只应在受信任的内网环境运行（能接触租户全部元数据）。
