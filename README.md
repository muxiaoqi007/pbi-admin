...
工具优先尝试**管理模式**（Admin API，全租户可见），当 Admin API 返回 401/403 时自动降级为**成员模式**（普通 API，仅当前身份可见的工作区）。部分世纪互联租户是否支持具体 Admin API 取决于租户、权限和区域能力。
...
| Admin API 401/403 | 检查租户设置、安全组、应用权限和当前认证方式 |
